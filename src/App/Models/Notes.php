...
class Notes {
  ...
  static function getShortNotes($user_id, $limit, $offset) {
    ...
    return self::query("SELECT id, title, created_at, updated_at FROM notes WHERE user_id = :user_id ORDER BY COALESCE(updated_at, created_at) DESC LIMIT $limit OFFSET $offset", $data);
  }
